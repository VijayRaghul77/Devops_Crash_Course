...
resource "azurerm_virtual_network" "dev-vm" {
...
resource "azurerm_subnet" "internal" {
  name                 = "internal"
  resource_group_name  = azurerm_resource_group.dev-rg.name
  virtual_network_name = azurerm_virtual_network.dev-vm.name
  address_prefixes     = ["${element(var.network_config, 1)}/${element(var.network_config, 2)}"]
}

resource "azurerm_network_interface" "dev-vm" {
...
resource "azurerm_virtual_machine" "dev-vm" {
  ...
  location              = azurerm_resource_group.dev-rg.location
  resource_group_name   = azurerm_resource_group.dev-rg.name
  network_interface_ids = [azurerm_network_interface.dev-vm.id]
